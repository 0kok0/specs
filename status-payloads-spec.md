...
If a user sends a new message before the messages are delivered that where sent while the user was offline, the new
message is supposed to be displayed last in a chat. This is where the basic algorithm of Lamport timestamp would fall
short, as it's only meant to order causally related events.

The status client therefore makes a "bid", speculating that it will beat the current chat-timestamp, s.t. the status client's
Lamport timestamp format is: `clock = `max({timestamp}{post-id}, chat_clock + 1)`
...
Messages with a `clock` less than `120` seconds under the whisper timestamp might indicate an attempt to insert messages in the chat history, this is not distinguishable though from a re-transmit from the `datasync` layer. A client MAY mark this messages with a warning to the user, or discard them.
...
`post-id` is the id of a message.
